...
import InputPointFix from "./InputPointFix";
import "./style.css";
...
function Right() {
  ...
  const { isInput, setIsInput, rightIsHidden, setRightIsHidden } =
  ...
  const nextStepClick = () => {
    ...
    const tagList = newTodoContent.tag;
      ...
      {isInput ? (
        <div className="flex justify-center">
          <div
            ...
            className="flex justify-start p-4 -mr-16 rounded-full w-7·3rem bg-gray-200 mt-2 h-12 transition-all hover:text-gray-400 text-gray-light p-2 text-center items-center cursor-pointer"
          >
            <Icon.ChevronLeft strokeWidth="3" size={26} />
          </div>
          <div
            onClick={nextStepClick}
            className="flex justify-center rounded-full w-7·3rem bgGrassGreen mt-2 h-12 transition-all hover:bg-green-700 text-white p-2 text-center items-center cursor-pointer"
          >
            下一步
